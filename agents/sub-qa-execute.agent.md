...
4. Branch on the smoke result before running anything else. **A failed smoke case never falls through to the full suite.**
  ...
  - Before classifying a missing feature locator, record the page's actual URL and the visible headings/accessibility snapshot from the trace. If the actual URL is the login path or the configured `signedInLocator` is absent, classify the failure as `ENVIRONMENT` (authentication was not established or persisted), not as a stale feature locator or product defect.
  ...
  - `HARNESS`: a selector, route, or fixture in the smoke spec is wrong. A generated page-readiness assertion (for example, a heading added only to prove the page loaded) that is not itself required by the TestRail expected result is always harness code, never product behavior. Compare it with the trace's rendered accessibility snapshot, correct that one thing to an exact observed locator, rerun the smoke case, and allow at most two such correction attempts. If the third smoke attempt still fails for a harness reason, halt with `STATUS: HARNESS_NOT_VIABLE`, name the unresolved selector or route, actual URL, and observed headings, and record no case results.
