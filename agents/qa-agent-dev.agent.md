...
16. **Test execution requires explicit human approval at two gates within Phase 3** — never proceed to browser execution without both:
   ...
   - **Gate 2, between test generation and test execution** (the Generated Tests Gate): after `sub-qa-generate-tests` returns and before invoking `sub-qa-execute`, the agent MUST present `QA-TESTS-{KEY}.md`'s Ground-Truth Verification table and invoke `sub-qa-notify` with `ACTION=AWAITING_QA_TEST_APPROVAL`. Every executable route, locator, semantic role, visible string, and required data value must already be confirmed against the configured live application. Human approval reviews verified tests; it never waives an unresolved executable detail. Skipping this gate defeats the purpose of the Ground-Truth Verification hard gate in `sub-qa-generate-tests`.
...
The worker must use its browser tools against `environment.applicationUrl` and authenticate from `environment.auth` when required. Repository source may support the check, but it cannot replace observation of rendered routes, accessibility semantics, visible strings, and table structure. If `sub-qa-generate-tests` returns `STATUS: GROUND_TRUTH_BLOCKED`, present the missing live observations and stop Phase 3. Do not invoke the Generated Tests Gate, ask the human to approve inferred details, or proceed to execution. Resume Step 3A only after the configured application or authentication is available for direct browser verification.
...
1. Verify every Ground-Truth Verification row has `Unresolved: None`. If any executable detail is unresolved, treat the worker result as `GROUND_TRUTH_BLOCKED` and do not open this gate. Otherwise present the `QA-TESTS-{KEY}.md` path, the count of tests generated, and the full `## Ground-Truth Verification` table to the human.
...
4. Approval cannot accept an unresolved route, locator, semantic role, visible outcome, or required test-data value. Send correction notes back to Step 3A or stop; never proceed with best-effort executable details.
