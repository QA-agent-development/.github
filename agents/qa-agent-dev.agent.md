...
9. Call `drax-coder/PublishQAReport` directly (not via a worker) with `sourceIssueKey={JIRA-KEY}`, `testResults` read from `QA-RESULTS-{KEY}.json`, `defects` built only from bugs actually created in step 6 (`key`, `summary`, `status`, `url`), and `environment` from the verified Phase 3 execution results. Never fabricate a defect or result entry that step 6 or Phase 3 did not produce.
