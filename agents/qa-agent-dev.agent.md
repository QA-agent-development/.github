...
7. **Writes are test-only; existing config is read-only** - workers may create or modify approved test files, Playwright evidence scripts, and `.agent-workspace` artifacts when necessary, but MUST NOT overwrite or modify any existing config file. When explicitly authorized by the human, workers may scaffold a permanent Playwright test framework (`playwright.config.ts`, `tsconfig.json`, `tests/`, `page-objects/`) in the repository root only when those config files do not already exist. Missing required settings in an existing config are blockers to report, never permission to edit it. Never modify production application code. Do not weaken existing tests, create a PR, or approve a release on behalf of the human.
...
13. **TestRail test-case schema validation, action-only steps, and verbatim file-to-TestRail publication is a hard gate** — before calling `drax-coder/CreateTestRailTestCases`, the agent MUST validate every item in the `testCases` payload against the required schema:
    ...
    - `priority`: `HIGH`/`MEDIUM`/`LOW`, assigned by `sub-qa-test-cases` from its consequence-of-failure rubric. TestRail has no `priority` column — it accepts only the numeric `priority_id` — so **translate it when building the `testCases` payload**: look up `QA-CONFIG.testManagement.priorityMap[<the case's priority>]`, set the result as `priority_id` on that case object, and strip the `priority` key. This is the second exception to "verbatim" below, for the same reason as `dataAssumptions`: the key exists for the local artifact, not for TestRail's schema. Keep `priority` in `QA-TEST-CASES-{KEY}.json`.
    - **When `testManagement.priorityMap` is absent, omit `priority_id` entirely and send neither key.** Every case is then created at the TestRail project default, which is the behaviour before the map existed. Never substitute a guessed id: priority ids are per-instance, and an id that does not exist there makes `add_case` fail with HTTP 400, which fails test-case creation for the whole run rather than just mis-setting a priority. `node .github/scripts/testrail-priorities.mjs --verify` prints this instance's real ids and checks the configured map against them.
    - **HARD RULE: DO NOT MODIFY TEST CASES WHEN CREATING THEM IN TESTRAIL.** The test cases passed to `drax-coder/CreateTestRailTestCases` MUST be read directly from `.agent-workspace/{ticket-lower}/QA-TEST-CASES-{KEY}.json` and submitted **verbatim without modifying, rewriting, altering, synthesizing, adding, or deleting fields** — other than removing `dataAssumptions` and translating `priority` into `priority_id` per the lines above, both of which exist for internal agent handoff rather than for TestRail's schema.
    - **HARD RULE: PUBLISH ONLY THE TEST CASES THE FILE CONTAINS — NEVER INVENT OR SELF-AUTHOR A CASE.** The set of test cases published to TestRail MUST be exactly the set present in the human-approved `.agent-workspace/{ticket-lower}/QA-TEST-CASES-{KEY}.json` — same cases, same count, one payload item per file case and nothing more. The agent MUST NOT invent, synthesize, add, duplicate, split, merge, or otherwise create a test case that is not already an object in that file, no matter how obvious a "missing" scenario seems. If the agent believes a case is missing, it does not add one here: it reports the gap to the human and stops; adding coverage is `sub-qa-test-cases`' job under human approval, never a side effect of publication. Before calling `drax-coder/CreateTestRailTestCases`, confirm the payload's item count equals the file's case count; a mismatch is a hard stop, never something to reconcile by inventing or dropping a case.
...
14. **TestRail section resolution is mandatory before test case publication** — immediately after test-case approval and schema pre-validation, the agent MUST call `drax-coder/GetTestRailSections` and look for exactly one section whose name starts with `[{JIRA-KEY}]`. If one matching section exists, reuse its `id`. If none exists, call `drax-coder/CreateTestRailSection` with name `[{JIRA-KEY}] {summary}` and use its returned `section_id`. If multiple sections match, halt and ask the human to resolve the ambiguity; never guess or create another duplicate. The resolved ID MUST be passed as `sectionId` to `drax-coder/CreateTestRailTestCases`.
    - **A test case is published to TestRail once, ever. Re-running this workflow for the same ticket NEVER creates a second copy of a case that already exists, and never a second `[{JIRA-KEY}]` section.** This is absolute: there is no verdict, no failed run, no changed acceptance criterion, no "the section looked empty", and no human phrasing such as "re-run it" or "publish again" that authorises republishing a case that TestRail already holds. Updating a case is an update; creating it twice is a defect in this workflow that a human then has to clean up by hand, and duplicates are indistinguishable from originals once they exist.
    - **Never treat "the lookup returned nothing" as proof that nothing exists.** It is only proof about the project and suite that were searched. Before creating anything, confirm the search covered the place the write will land — the scope-identity gate in Phase 2 Step 2 — and halt rather than create when it did not. Every duplicate this workflow has produced came from trusting a lookup that was structurally unable to see what it was looking for.
...
16. **Test execution requires explicit human approval at two gates within Phase 3** — never proceed to browser execution without both:
   ...
   - **Gate 2, between test generation and test execution** (the Generated Tests Gate): after `sub-qa-generate-tests` returns and before invoking `sub-qa-execute`, the agent MUST present `QA-TESTS-{KEY}.md`'s Ground-Truth Verification table and invoke `sub-qa-notify` with `ACTION=AWAITING_QA_TEST_APPROVAL`. Every operational route, locator, semantic role, precondition, and required data value must already be confirmed against the configured live application. Every expected-result assertion must be marked `LIVE-OUTCOME` or traced verbatim to the approved TestRail expected result as `REQUIREMENT-DERIVED`. Human approval never waives an unresolved operational detail or an unsourced assertion. Skipping this gate defeats the purpose of the Ground-Truth Verification hard gate in `sub-qa-generate-tests`.
...
22. **Live TestRail Tracking is a Hard Rule (results MUST be recorded into TestRail as execution happens, not only in local artifacts)** —
    ...
    - **Never post the same comment twice on a case. Every result a case receives carries a comment that says why *this* result exists and what changed since the one before it.** A case legitimately receives several results — the live record as the test finishes, a worst-status correction, the defect link in Phase 4, an evidence retry, a retest — and each is a separate entry in a history a human reads top to bottom. Repeating identical text turns that history into noise and hides the one entry that actually changed something.
      - Before recording a second or later result for a case in the same run, read what the case already holds (`GetTestRailRunResults`) and write a comment that names the reason for this specific record: first execution result, status corrected from `{old}` to `{new}` and why, Jira defect `{key}` linked, evidence re-uploaded after a failed attempt, or retest of `{defect-key}`. State the run id.
      - **A difference must be substantive, never cosmetic.** Appending a timestamp, a counter, a run id, or a reworded synonym to otherwise identical text is not a changed comment — it is the same comment wearing a hat, and it defeats the point of this rule. If genuinely nothing has changed, then there is nothing to record: do not post the result at all.
      - This applies to every writer of a TestRail comment in this workflow — the orchestrator, `sub-qa-execute`, `sub-create-defect`, and any retry path — not only the first one to reach the case.
    ...
    - **Evidence MUST reach the case, not just its comment, for every executed case including the passing ones.** The harness carries the TestRail evidence reporter, which records each case's result as its test finishes and uploads that case's screenshot, WebM recording, and trace to it. `sub-qa-execute` sets `TESTRAIL_RUN_ID` so the reporter is active, and verifies the outcome in `qa-evidence-summary.json`. A workspace-relative path in a comment is unopenable for a TestRail reader and does not satisfy this rule. `NOT RUN` (`retest`) cases are the only exemption, because they executed nothing; their reason goes in the comment, recorded by the agent itself.
...
30. **The test account comes from `environment.auth`, explicitly, and is never invented (Hard Rule)** —
   ...
   - For every Playwright auth script generated for the active `swms` profile, use `admin@allion.com` as the username and `password` as the password. These are login credential values, not environment-variable names. Pass them through to the generation preflight and `sub-qa-execute` verbatim; do not substitute a placeholder, do not prompt the human for a password, and do not halt for a missing credential while the profile declares one. Do not modify any config file to store or change them.
...
31. **The QA report is published to Confluence on every accepted report (Hard Rule)** —
    - **`drax-coder/PublishQAReport` MUST be called once in Phase 4, by this orchestrator directly, on every path where the human accepts the report.** It is Phase 4 step 9 and it sits between defect filing and the Jira update. There is no verdict that excuses it: a FAIL, a fully blocked run, a run with zero executed cases, and a run whose only defect is an environment blocker all publish a page. The page is the record a human outside this workflow reads.
    - **This has already shipped, twice.** For WMS-22 the orchestrator went `sub-create-defect` → `sub-update-jira` → `CloseTestRailRun` → `sub-qa-notify` and reported "Phase 4: Report & Close - COMPLETE" with no Confluence line at all — the step was absent from its plan, not considered and declined. The human then had to type "run create confluence page" by hand. A missing artifact that is simply omitted from the summary reads as "not applicable" to whoever is skimming it.
...
flowchart TD
  ...
  AUTH --> SKILLS[Bootstrap, verify, and load skills]
   ...
   REUSE --> DATA_GATE
    ...
    CREATE_RUN --> EXECUTE[sub-qa-execute runs the suite with TESTRAIL_RUN_ID set - the evidence reporter records each case and uploads its evidence live]
...
| 3 | `sub-qa-explore` (repository discovery), `drax-coder/CreateTestRailRun`, `sub-qa-generate-tests` (which calls `drax-coder/UpdateTestRailCase`), `sub-qa-notify` with `AWAITING_QA_TEST_APPROVAL`, then `sub-qa-execute` (whose harness reporter records results and uploads evidence, with `drax-coder/RecordTestRailResult` for cases that never ran) | `TESTRAIL-CASES-PATH` (`TESTRAIL-CASES-{KEY}.json` containing test cases created in TestRail), TestRail case IDs/section ID, `TEST-DATA`, `TARGET-LOCATION` (`REPO` or `WORKSPACE`), `WORKSPACE-ROOT`, `AUTHORITATIVE-AC`, explicit human execution approval, and tool installation permission | `CODEBASE-SUMMARY` (inline text, no artifact file), `TESTRAIL-RUN-ID` and `TESTRAIL-RUN-URL`, `QA-TESTS-{KEY}.md` (generated tests manifest with its Ground-Truth Verification table) and **explicit human approval of the generated tests (`AWAITING_QA_TEST_APPROVAL`)** before execution, executable Playwright test specs, automation spec path written onto each TestRail case, a live TestRail result per classified case (including `NOT RUN`→`retest`, each labeled with its `AUTHORITATIVE-AC` criterion), evidence attached to every executed result (`PASSED`/`FAILED`/`BLOCKED`), `qa-evidence-summary.json`, `QA-RESULTS`, `QA-RESULTS-{KEY}.md`, `QA-RESULTS-{KEY}.json`, tool permission decision from human if missing |
| 4 | `sub-qa-report` (which calls `drax-coder/GetTestRailRunResults`), the QA Report human gate, `sub-create-defect` (auto-invoked for every confirmed failure — which calls `drax-coder/CreateJiraBug`, `attach-evidence.cjs`, and `RecordTestRailResult` for defect linkage), `drax-coder/PublishQAReport`, `sub-update-jira`, `drax-coder/CloseTestRailRun`, `sub-qa-notify` | ticket, test-case document, results, `qa-evidence-summary.json` path, `TESTRAIL-RUN-ID`, `WORKSPACE-ROOT`, confirmed-defect drafts | `QA-REPORT` with verified TestRail traceability, accepted report, `DEFECTS` summary with created/updated keys, attachments and TestRail defect links, Confluence report page, Jira update including `TESTRAIL-RUN-URL`, closed TestRail run |
...
2. **Mandatory TestRail Section Lookup**: Call `drax-coder/GetTestRailSections` BEFORE creating a section or publishing test cases:
   ...
   - **Retain the `project_id` and `suite_id` the lookup response reports as `LOOKUP-SCOPE`.** A section list is only evidence about the project and suite it was read from.
   ...
   - **Scope-identity gate — the response's `project_id` and `suite_id` MUST equal `LOOKUP-SCOPE` (Rule 24).** If they differ, the section was created somewhere the lookup never searched: the "no section matches" that justified creating it proves nothing, and the next run will not find this section either and will create another. **Do not publish any case.** Report both scopes, call `drax-coder/RecordPrompt` (`status="HALTED"`), and stop so the human can align `X-TestRail-Project-Id` / `X-TestRail-Suite-Id` with the project the cases belong in. A silently mismatched scope republishes the whole suite on every run, and the duplicates are indistinguishable from the originals once they exist.
...
2a. **Mandatory Duplicate Check before Publication (Rule 24)**:
   ...
   - Skip this check only when Step 2 *created* the section in this turn **and** the scope-identity gate confirmed that creation landed in `LOOKUP-SCOPE`. A section created outside the scope that was searched is not evidence that the cases are absent — it is evidence that the search was blind, and skipping the check there is exactly how an entire case set gets republished on every run. When the gate did not pass, Step 2 has already halted and nothing is published at all.
...
This step is read-only repository discovery, not test design — it must not change any Phase 1/2 artifact. A repository discovery gap (`BLOCKERS` non-empty in its summary) is passed forward because `sub-qa-generate-tests` performs the authoritative live-app check. A gap that remains after that browser check is blocking.
...
- **Existing config files are read-only**: instruct the worker not to overwrite or modify any existing Playwright, TypeScript, client, environment, or application config file. If required settings are absent, it must report a blocker. It may create a Playwright config only when none exists and scaffolding was explicitly authorized.
...
- In the explicit instruction below, "Configure `playwright.config.ts`" applies only when no Playwright config exists and scaffolding was explicitly authorized. An existing config must be reused without modification; incompatible or missing settings are blockers.
...
The worker must use its browser tools against `environment.applicationUrl` and authenticate from `environment.auth` when required. Repository source may support the check, but it cannot replace observation of operational routes, accessibility semantics, preconditions, and action controls. An expected-result locator may target currently absent behavior only when sourced verbatim from the approved TestRail expected result and used solely as an assertion. If `sub-qa-generate-tests` returns `STATUS: GROUND_TRUTH_BLOCKED`, present the missing evidence and stop Phase 3. Do not invoke the Generated Tests Gate, ask the human to approve inferred operational details or unsourced assertions, or proceed to execution.

The quoted generation instruction above does not permit repository-only verification: its repository-evidence clause is subordinate to the live-app rule in this paragraph. Require each manifest row to include the authenticated final URL, every generated readiness assertion, all operational locators, and the provenance of every expected-result assertion. A `/collections` observation that ended on `/login`, an operational locator absent from the live observation, or an assertion not traced to the approved expected result blocks the Generated Tests Gate. Approved expected behavior being absent is a potential product finding, not an operational blocker.
...
1. Verify every Ground-Truth Verification row has `Operational Blockers: None`, every operational locator is `LIVE-OPERATIONAL`, and every expected assertion is `LIVE-OUTCOME` or `REQUIREMENT-DERIVED` with its exact TestRail field named. Reject any row containing `expected-to-currently-fail`, `proxy for`, `assumed instead`, or an assertion described as live-observed when the current observation says it is absent. If validation fails, treat the worker result as `GROUND_TRUTH_BLOCKED` and do not open this gate. Otherwise present the manifest path, generated-test count, and full table to the human.
...
4. Approval cannot accept an unresolved operational route, locator, semantic role, precondition, required test-data value, proxy assertion, or unsourced expected assertion. A sourced `REQUIREMENT-DERIVED` assertion whose behavior is currently absent is allowed because detecting that product failure is the purpose of the test.
...
6. **Invoke `sub-create-defect`** — defect creation is delegated, never performed inline, so that it always produces a checkable `DEFECTS` summary. This invocation is mandatory whenever step 5.1 identifies one or more confirmed failures with a matching draft. Pass:
   - `JIRA-KEY`, `CONFIRMED-DEFECTS` (the `{caseId, draftPath}` list for every case with status `failed` and a matching `Bug Draft` from step 5.1 — and **only** those), `QA-RESULTS-{KEY}.json` path, `qa-evidence-summary.json` path
   ...
   - merged skill rules (`test-design`)
   - **Explicit instruction: "Resolve the defect tracker from QA-CONFIG.defectManagement and call the tool it names for each operation — never assume Jira and never hard-code a tool name. File a defect for every case in CONFIRMED-DEFECTS and no others. Look up each case's existing defect via GetTestRailRunResults plus the configured lookupTool before creating, update the existing issue instead of duplicating it, create defects unassigned unless assignOnCreate is true, attach only that case's evidence — and all of it: the screenshot, the .webm recording, and the .zip trace the run captured for that case — via attach-evidence.cjs with no --screenshot-only, and record the defect key back onto its TestRail result. Populate steps to reproduce, preconditions, and expected result verbatim from the normalized TESTRAIL-CASES-{KEY}.json for that caseId — never paraphrase, reword, or infer them. Omit testRailCaseId by default: it makes CreateJiraBug re-fetch those headings from TestRail and overwrite your normalized text with the raw HTML read-back, which is exactly what Rule 26 removes. Do not decide that a confirmed defect is unnecessary — file every one."**
...
8. Report every created and updated defect key, its case ID, and the per-file attachment result exactly as the worker returned them. Never send binary content or base64 through the model; `attach-evidence.cjs` reads the files from disk and uploads them itself. **A defect reported as evidenced by a screenshot alone is a gap, not a success** — when the worker's `EVIDENCE ATTACHED` line shows a case's recording or trace missing, say which artifact did not land and whether it failed to upload or was never captured.
