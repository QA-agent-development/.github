...
8. Report every created and updated defect key, its case ID, and the per-file attachment result exactly as the worker returned them. Never send binary content or base64 through the model; `attach-evidence.cjs` reads the files from disk and uploads them itself.
