...
For each confirmed defect, add a `Bug Draft` subsection containing the proposed Jira summary, ordered report sections, priority, labels, source case IDs, exact evidence paths from `qa-evidence-summary.json`, and the Playwright trace viewer command. Never claim an artifact exists unless that summary lists it. Browser defects should reference their generated Playwright script and available screenshot, recording, and trace; non-browser defects may have no visual evidence.
