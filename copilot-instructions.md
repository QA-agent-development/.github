...
| `.github/client-config/` | Client profiles (`<client>.yaml`), `defaults.yaml`, and the `schema.json` they validate against. The active profile supplies the tracker, the environment, and the credentials contract (Rule 30). |
...
The system under test is whatever `environment.applicationUrl` in the active
client profile names, and the profile is the only authority on it. It may be an
application in this repository or one that lives and runs entirely elsewhere.
When it is not here, that is the configuration working as intended: never report
the absence of application source as a finding, and never infer the
application's location, stack, or structure from the repository layout.
